...
const JungleMonkeyAdventure: React.FC<GameProps> = ({ gameName }) => {
    const [heroY, setHeroY] = useState(GAME_HEIGHT - HERO_SIZE);
    const [heroVelocity, setHeroVelocity] = useState(0);
    const [obstacles, setObstacles] = useState<{x: number, height: number, type: 'snake' | 'banana' }[]>([]);
    ...
    const startGame = () => {
        ...
        setHeroVelocity(0);
        setObstacles([{ x: GAME_WIDTH, height: 40, type: 'snake' }]);
        setScore(0);
    ...
    useEffect(() => {
        ...
        gameLoopRef.current = requestAnimationFrame(function gameLoop() {
            ...
            setObstacles(obs => {
                ...
                for (const o of newObstacles) {
                    if (o.x < -OBSTACLE_WIDTH) continue; // remove if offscreen
                    ...
                    if (heroRect.x < obstacleRect.x + obstacleRect.width &&
                        ...
                        if (o.type === 'snake') {
                            ...
                            setScore(s => s + 10);
                            // don't add banana to kept obstacles
                            continue;
                        }
                    }
                     keptObstacles.push(o);
                }
                return keptObstacles;
            });
        ...
        obstacleTimeoutRef.current = window.setInterval(() => {
            const type = Math.random() > 0.3 ? 'snake' : 'banana';
            setObstacles(obs => [...obs, { x: GAME_WIDTH, height: 40, type }]);
        }, 1500);
    ...
    return (
        <div className="max-w-4xl mx-auto text-center bg-white p-6 rounded-2xl shadow-2xl border-4 border-emerald-300">
            ...
            <div
                onClick={jump}
                className="relative bg-lime-200 w-full overflow-hidden cursor-pointer border-2 border-gray-400"
            ...
            >
                ...
                <div className="absolute text-4xl" style={{ left: 50, bottom: (GAME_HEIGHT - heroY - HERO_SIZE) }}>🐒</div>
                {obstacles.map((obs, i) => (
                    <div key={i} className="absolute text-4xl" style={{ left: obs.x, bottom: 0, height: obs.height }}>
                        {obs.type === 'snake' ? '🐍' : '🍌'}
                    </div>
                ))}
...
export default JungleMonkeyAdventure;